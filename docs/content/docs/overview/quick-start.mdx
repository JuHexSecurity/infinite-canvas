...
```bash
git clone git@github.com:JuHexSecurity/infinite-canvas.git
...
Canvas projects and My Assets are primarily stored in the browser. In browser-direct mode, the API key is also stored locally and used by the browser to call OpenAI-compatible endpoints directly; team deployments should enable the private AI gateway described in the Docker deployment guide.
