...
import { buildApiUrl, isServerManagedKeyDeployment, withLocalProxy, type AiConfig, type ModelCapability } from "@/stores/use-config-store";
...
function createPluginHttp(config: AiConfig, options?: RequestOptions): PluginHttp {
    const run = async (method: "get" | "post", path: string, body: unknown, opts?: PluginHttpOptions) => {
        ...
        const response = await axios.request({
            ...
            params: opts?.params,
            headers: pluginHeaders({ ...(isServerManagedKeyDeployment() ? {} : { Authorization: `Bearer ${config.apiKey}` }), ...opts?.headers }, method === "post" && !isForm && body !== undefined),
